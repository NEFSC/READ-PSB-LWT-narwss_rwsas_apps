...
observeEvent(input$rawupload, {
  ...
  if (input$sd == "") {
    ...
    output$error2 <-
      renderText({
        "Uh oh! Those files can't be found! Double check your connection to the network, the local network pathway, your data, and/or your survey date entry."
             ...
             !file.exists(paste0(path, survey_date, '/', 'effsig_', survey_date, '.csv'))) {
    output$error2 <-
      renderText({
        "No initial eff/sig edits were saved."
      })
  } else {
    ...
    eff_sig$SIGHTING_COMMENTS <-
      as.character(eff_sig$SIGHTING_COMMENTS)
    ...
    output$handsES = renderRHandsontable({
      handsES
    })
    enable("edittable")
  }
...
observeEvent(input$edittable, {
  if (input$sd == "") {
    output$error <-
      renderText({
        "You must edit your raw eff and sig files first"
      })
  } else {
    output$error <- renderText({
      ""
    ...
    for (i in 1:nrow(f))
      if (!is.na(f$SPCODE[i]) &&
          grepl('psi', f$SIGHTING_COMMENTS[i])) {
        ID_RELIABILITY[i] = 2
    ...
    for (i in 1:nrow(f))
      if (!is.na(f$SPCODE[i]) &
          grepl('no ph', f$SIGHTING_COMMENTS[i])) {
        PH[i] = 1
      } else if (!is.na(f$SPCODE[i]) &&
                 (grepl('thru', f$SIGHTING_COMMENTS[i]) |
                  (grepl('ph', f$SIGHTING_COMMENTS[i])))) {
        PH[i] = 2
      } else if (!is.na(f$SPCODE[i])) {
        PH[i] = 1
      } else {
    ...
    for (i in 1:nrow(f))
      if (!is.na(f$ACTUAL_HEADING[i]) &&
          (f$ACTUAL_HEADING[[i]] + round(f$HEADING[[i]], digits = 0) <= n)) {
        f$ACTUAL_HEADING[[i]] = f$ACTUAL_HEADING[[i]] + round(f$HEADING[[i]], digits = 0)
      } else if (!is.na(f$ACTUAL_HEADING[i]) &&
                 (f$ACTUAL_HEADING[[i]] + round(f$HEADING[[i]], digits = 0)) >= n) {
    ...
    LEGSTAGE = NULL
    for (i in 1:nrow(f))
      if (grepl('off watch', f$EFFORT_COMMENTS[i]) |
          grepl('no res', f$EFFORT_COMMENTS[i])) {
        LEGSTAGE[i] = 0
    ...
    ##alt obs, ap, fin est ----
    f$LEGSTAGE[which(grepl('alt obs', f$SIGHTING_COMMENTS) &
                       f$LEGSTAGE != 0)] <- 6
    f$PSB_LEGSTAGE[which(grepl('alt obs', f$SIGHTING_COMMENTS))] <-
      6
    f$PSB_LEGSTAGE[which(
      grepl('ap', f$SIGHTING_COMMENTS) &
        !grepl('no ap', f$SIGHTING_COMMENTS) &
        !grepl('unsure', f$SIGHTING_COMMENTS)
    )] <- 7
    f$PSB_LEGSTAGE[which(grepl('fin est', f$SIGHTING_COMMENTS))] <-
      9
    ...
    factpal <-
      colorFactor(palette = "Set1", domain = sightings$SPCODE)
    egmap <- leaflet(data = rf) %>%
      ...
      addCircleMarkers(
        lng = ~ sightings$LONGITUDE,
        lat = ~ sightings$LATITUDE,
        color = ~ factpal(sightings$SPCODE),
        stroke = FALSE,
        fillOpacity = 2,
        radius = 5
      ) %>%
      addLegend(pal = factpal,
                values = sightings$SPCODE,
                opacity = 1) %>%
      addWMSTiles(
    ...
    output$egmap = renderLeaflet({
      print(egmap)
    })
    output$handsrf = renderRHandsontable({
      handsrf
    })
    enable("save")
  }
  ...
  observeEvent(input$save, {
    ...
    if (criteria$loc == 'Network') {
      path <-
        #paste0('//net/mmi/Fieldwrk/Aerials/20',   #previous location on net
        paste0('/mnt/PSD-Whale_Surveys/Fieldwrk/Aerials/20',      #added 20250725 for new nefscdata location
               yr,
               '/Flights/edit_data/')
      enable("sas")
    ...
    write.csv(
      final,
      paste0(path, survey_date, '/', 'f_', survey_date, '.csv'),
      na = '',
      row.names = FALSE
    )
    ...
    for (i in 1:seq_along(nrow(egtable)))
      if (nrow(egtable) == 0) {
        ...
        for (i in 1:nrow(egtable))
          if (egtable$B1_FINAL_CODE[i] == '00' |
              egtable$B1_FINAL_CODE[i] == '54' |
              egtable$B1_FINAL_CODE[i] == '90' |
              egtable$B1_FINAL_CODE[i] == '92') {
            egtable$B1_FINAL_CODE[i] = egtable$B1_FINAL_CODE[i]
          } else {
            egtable$B1_FINAL_CODE[i] = ""
          }
        for (i in 1:nrow(egtable))
          if (egtable$B2_FINAL_CODE[i] == '00' |
              egtable$B2_FINAL_CODE[i] == '54' |
              egtable$B2_FINAL_CODE[i] == '90' |
              egtable$B2_FINAL_CODE[i] == '92') {
            egtable$B2_FINAL_CODE[i] = egtable$B2_FINAL_CODE[i]
          } else {
            egtable$B2_FINAL_CODE[i] = ""
          }
        for (i in 1:nrow(egtable))
          if (egtable$B3_FINAL_CODE[i] == '00' |
              egtable$B3_FINAL_CODE[i] == '54' |
              egtable$B3_FINAL_CODE[i] == '90' |
              egtable$B3_FINAL_CODE[i] == '92') {
            egtable$B3_FINAL_CODE[i] = egtable$B3_FINAL_CODE[i]
          } else {
            egtable$B3_FINAL_CODE[i] = ""
          }
        for (i in 1:nrow(egtable))
          if (egtable$B4_FINAL_CODE[i] == '00' |
              egtable$B4_FINAL_CODE[i] == '54' |
              egtable$B4_FINAL_CODE[i] == '90' |
              egtable$B4_FINAL_CODE[i] == '92') {
            egtable$B4_FINAL_CODE[i] = egtable$B4_FINAL_CODE[i]
          } else {
            egtable$B4_FINAL_CODE[i] = ""
          }
        for (i in 1:nrow(egtable))
          if (egtable$B5_FINAL_CODE[i] == '00' |
              egtable$B5_FINAL_CODE[i] == '54' |
              egtable$B5_FINAL_CODE[i] == '90' |
              egtable$B5_FINAL_CODE[i] == '92') {
    ...
    Behavior <-
      as.character(
        paste0(
          egtable$B1_FINAL_CODE,
          " ",
          egtable$B2_FINAL_CODE,
          " ",
          egtable$B3_FINAL_CODE,
          " ",
          egtable$B4_FINAL_CODE,
          " ",
          egtable$B5_FINAL_CODE
        )
      )
    ...
    egrep <- egtable %>%
      filter((
        PSB_LEGSTAGE == 7 |
          startsWith(SIGHTING_COMMENTS, "fin est no break") |
          grepl('No right whales', egtable$DateTime)
      ))
    #print(egrep)
    egrep <- egrep %>%
      filter(
    ...
    noneg <-
      filter(confsig,
             confsig$SPCODE != 'RIWH' & grepl('WH', confsig$SPCODE))
    
    ##must do it this way to get the right number of right whales
    noeg <-
      subset(noneg,
             select = c("SPCODE", "GROUP_SIZE", "LATITUDE", "LONGITUDE"))
    yeseg <-
      subset(egrep,
             select = c("SPCODE", "GROUP_SIZE", "LATITUDE", "LONGITUDE"))
    ...
    if (nrow(netable) == 0) {
      ...
      netable$SPCODE[which(netable$SPCODE == 'GRAM')] <-
        "Risso's Dolphin"
      netable$SPCODE[which(netable$SPCODE == 'GRTU')] <-
        "Green Seaturtle"
      netable$SPCODE[which(netable$SPCODE == 'HAPO')] <-
        'Harbor Porpoise'
      netable$SPCODE[which(netable$SPCODE == 'HHSH')] <-
        'Hammerhead Shark'
      netable$SPCODE[which(netable$SPCODE == 'HUWH')] <-
        'Humpback Whale'
      netable$SPCODE[which(netable$SPCODE == 'KIWH')] <-
        'Killer Whale'
      netable$SPCODE[which(netable$SPCODE == 'LETU')] <-
      ...
      netable$SPCODE[which(netable$SPCODE == 'SONO')] <-
        "Sonobuoy Deployed"
      ...
      netable$SPCODE[which(netable$SPCODE == 'WSDO')] <-
    ...
    if (file.exists('./scripts/oracleaccess.R') == TRUE){
      ...
      if (nrow(egtable) == 0 |
          egtable$DateTime[1] == 'No right whales sighted') {
        ##if no Eg:
        output$error1 <-
          renderText({
        ...
        output$obspeeps_options <- renderUI({
          radioButtons(
            ...
            choiceValues = list(880, 3, 850, 4, 935, 940, 2, 1, 0),
        ...
        output$plane_options <- renderUI({
          radioButtons(
            ...
            choiceValues = list(183, 178, 284, 49, 18),
            selected = "An Otter"
          )
        })
    ...
    date_formats$month1 <-
      month.abb[month(as.character(final$DateTime[1]))]
    ...
    reportleaf <- reportleaf %>%
      ...
      addCircleMarkers(
        lng = ~ LONGITUDE,
        lat = ~ LATITUDE,
        color = ~ leafpal(SPCODE),
        stroke = FALSE,
        fillOpacity = 2,
        radius = 5
      ) %>%
      addLegend(pal = leafpal,
                values = spectab$SPCODE,
                opacity = 0.9) %>%
      addWMSTiles(
        "https://gis.ngdc.noaa.gov/arcgis/services/graticule/MapServer/WMSServer/",
        layers = c("1-degree grid", "5-degree grid"),
    ...
    enable("report")
    enable("report_html")
    output$reportmap = renderLeaflet({
      print(reportmap)
    })
    output$netable <- renderTable({
      netable
    }, digits = 0)
    output$egreport <- renderTable({
      egreport
    })
    print("html1")
