...
\newline
On `r params$triggerdateletter`, `r params$triggerword` North Atlantic right whales were sighted `r params$letterdirect` of *landmark* by *User defined, give suggestions*.  *Since no pretections are in place in this region at this time, we recommend a DMA be initiated / We therefore recommend extending the existing DMA* that is bounded by:
