...
observeEvent(input$query,{
  ...
  #disable("kml")
      ...
      sasdma<-leaflet(data = smapresent.sp, options = leafletOptions(zoomControl = FALSE)) %>% 
        ...
        addLegend(title = "Dynamic Management: solid border = DMA, dashed border = Acoustic", colors = c("yellow","orange","red"), labels = c("Active zone","Active zone eligible for extension","SMA"), opacity = 0.4, position = "topleft")
      
      dma_react$sasdma = sasdma

      output$sasdma = renderLeaflet({print(dma_react$sasdma)})
      output$actdma<-renderTable({actdma},  striped = TRUE)
      output$actdma_bounds<-renderTable({actdma_bounds%>%arrange(ID,VERTEX)},  striped = TRUE)
